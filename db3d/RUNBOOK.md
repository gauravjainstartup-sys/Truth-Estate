...
### ⚡ Fast path — all the DB data in one shot (Steps 1 · 2 · 5 combined)

Everything that gets **saved to the database** — schema, the intake table, all
three project seeds, the demo grants and pipeline status — is bundled, in
dependency order, into one idempotent file: **`db3d/apply-all.sql`** (~256 KB,
generated by `node db3d/build-apply-all.mjs`; re-run safe).

```bash
# CLI (recommended for 256 KB — get $DB_URL from Dashboard → Settings → Database):
psql "$DB_URL" -f db3d/apply-all.sql
# …or paste db3d/apply-all.sql into the SQL editor and Run.
```

Prerequisite you already created: the **`project_input_feed` view** (it reads
your `backlog_*` tables and is not in this file). After the one-shot, skip to
**Step 3 (secrets)** — the schema/data/grants steps below are what it contains,
kept for reference and for running them piecemeal.
