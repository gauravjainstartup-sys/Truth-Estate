...
export default function ReportConstruction({ p }: { p: ProjectIntel }) {
  ...
  return (
    <div className="mt-8">
      ...
      {/* Construction vs RERA plan — one verdict chip (state + size + stakes),
          one labelled axis tick. The bar shows it; the chip says it; nothing twice. */}
      <div className="mt-5 rounded-2xl border border-[#1a1a1a]/8 bg-white/60 p-6 md:p-7">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[0.72rem] font-semibold ${o.aheadOfPlan >= 0 ? "border-[#238c55]/25 bg-[#238c55]/[0.1] text-[#1c7a4c]" : "border-[#b0503e]/30 bg-[#b0503e]/[0.08] text-[#9a4130]"}`}>
            {o.aheadOfPlan === 0
              ? <>● On the RERA plan</>
              : <>{o.aheadOfPlan > 0 ? "▲" : "▼"} {Math.abs(o.aheadOfPlan)} pts {o.aheadOfPlan > 0 ? "ahead" : "behind"}{o.ahead !== 0 && <> · ~{Math.abs(o.ahead)} mo {o.ahead > 0 ? "banked" : "at risk"}</>}</>}
          </span>
        </div>
        <div className="relative mt-6 h-9 rounded-lg bg-[#f0ebe1]">
          <div className="absolute inset-y-0 left-0 overflow-hidden rounded-l-lg" style={{ width: `${o.expectedPct}%`, background: "repeating-linear-gradient(45deg,#e6ddc9,#e6ddc9 6px,#efe8d8 6px,#efe8d8 12px)" }} />
          <div className="absolute inset-y-0 left-0 rounded-l-lg" style={{ width: `${o.actualPct}%`, background: "linear-gradient(90deg,#1e6b45,#238c55)" }} />
          ...
          {/* the RERA line — where the build should stand today */}
          <span aria-hidden className="absolute -bottom-1.5 -top-1.5 w-[2px] rounded bg-[#1a1a1a]/60" style={{ left: `calc(${o.expectedPct}% - 1px)` }} />
        </div>
        <div className="relative mt-2 h-5 text-[0.7rem]">
          <span className="absolute -translate-x-1/2 whitespace-nowrap text-[#1a1a1a]/45" style={{ left: `${o.expectedPct}%` }}>
            <b className="font-semibold text-[#1a1a1a]/75">{o.expectedPct}%</b> RERA-due today
          </span>
        </div>
      </div>
