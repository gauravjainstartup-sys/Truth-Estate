...
/* Chapter I — the homes. Per-configuration cards: a 2D floor plan (a
   licensed image where we have one, else an indicative zoning schematic),
   the areas as measured, the derived efficiency read, and the ticket. The
   dimensioned plan + 3D walkthrough + sun/air/Vastu live inside Unit
   Intelligence — the gated layer. */
  ...
  return (
    <div>
      <p className="-mt-2 mb-6 max-w-2xl text-[0.92rem] font-light leading-[1.7] text-[#1a1a1a]/55">
        The home as your agreement will measure it — the plan, the carpet, and what the gap to super area costs you. Indicative; exact areas vary by tower and stack.
      </p>
              ...
              <div className="grid gap-5 p-6 lg:grid-cols-[1.2fr_1fr]">
                ...
                <div>
                  <div className="overflow-hidden rounded-xl border border-[#1a1a1a]/10 bg-[#FBF8F2]">
                    {h.plan ? (
                      <img src={`${basePath}/${h.plan}`} alt={`${h.config} floor plan — ${p.name}`} className="block w-full" />
                    ) : (
