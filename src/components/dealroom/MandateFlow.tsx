...
/* ════════════════════════════════════════════════════════════════
   THE DEAL ROOM — MANDATE FLOW (Stage-1 demand experiment)

   The three-step mandate the buyer commissions, opened in-page from the
   /deal-room landing (there is no separate /deal-room/mandate route):
...
/* True when a Google round-trip stashed a draft and the reader came back
   signed in — the landing uses this to re-open the flow so the mount effect
   below can submit the stashed mandate and land on the confirmation. */
export function hasPendingMandate(): boolean {
  try { return !!window.localStorage.getItem(DRAFT_KEY) && isSignedIn(); }
  catch { return false; }
}

export default function MandateFlow({ onExitToLanding }: { onExitToLanding: () => void }) {
  ...
  /* On mount: this component only exists once the flow is open, so count a
     start. Then resume a Google round-trip — if a draft was stashed before the
     redirect and we're back signed in, submit it and show the confirmation. */
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#14110d] text-[#f4efe6]" style={{ fontFeatureSettings: '"ss01"' }}>
      ...
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-6 py-6 md:px-10">
        ...
        <button onClick={onExitToLanding} className="font-mono text-[0.7rem] uppercase tracking-[0.14em] text-[#a9a196] transition-colors hover:text-[#f4efe6]">← Back to the Deal Room</button>
      </nav>
              ...
              {step === 0 && (
                <div>
                  ...
                  <div className="mt-9 flex justify-between">
                    <button onClick={onExitToLanding} className="text-[0.85rem] text-[#a9a196] hover:text-[#f4efe6]">← Back</button>
      ...
      {screen === "done" && (
        <div className="mx-auto max-w-xl px-6 py-20 text-center md:px-10">
          ...
          <div className="mt-9 flex justify-center gap-4">
            ...
            <button onClick={onExitToLanding} className={btnGhost}>Back to the Deal Room</button>
          </div>
        </div>
      )}
    </div>
