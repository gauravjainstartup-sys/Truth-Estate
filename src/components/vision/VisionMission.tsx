...
/* Coverage counts come from the database, not from copy. A public brand page
   that names a number has to be the number we can actually stand behind, and
   a hardcoded one goes stale the week a project is added. When a count is
   missing the sentence still reads correctly — it stops naming the figure
   rather than printing a zero. */
export type VisionCoverage = {
  activeProjects?: number | null;
  microMarkets?: number | null;
  developers?: number | null;
};

function coverageScope(c: VisionCoverage): string {
  const projects =
    c.activeProjects && c.activeProjects > 0
      ? `${c.activeProjects} under-construction projects`
      : "every under-construction project we track";
  const markets =
    c.microMarkets && c.microMarkets > 0
      ? ` in ${c.microMarkets} Gurugram micro-markets`
      : " in Gurugram";
  const devs =
    c.developers && c.developers > 0
      ? `, built by the ${c.developers} developers who account for most of that supply`
      : "";
  return `${projects}${markets}${devs}`;
}
...
const MANIFESTO_TRUTHS: { roman: string; truth: string; rationale: string }[] = [
  ...
  {
    roman: "IV",
    truth: "Information asymmetry is where buyers lose the most money.",
    rationale:
      "When developers hold the facts and buyers hold marketing gloss, capital is misallocated. Radical transparency levels the table.",
  },
  {
    roman: "V",
  ...
  },
  {
    ...
    rationale:
      "We do not sell user data, and we never pass a buyer's details to a developer unless that buyer asks us to. Every line of code and research memo is built to protect the buyer's balance sheet.",
  },
  {
    roman: "VII",
  ...
  return [
    {
      ...
      description: `We track civil construction pacing, contractor appointments, structural slab velocity, and statutory filings across ${coverageScope(
        c,
      )}. Every claim is checked against RERA filings and public regulatory records.`,
    },
    {
      number: "02",
    ...
    },
    {
      ...
      subtitle: "Institutional depth, at family scale",
      description:
        "Institutional investors commission deep forensic due diligence before they commit capital. A family putting its life savings into a single apartment deserves that same depth—not a brochure.",
    },
    {
      number: "05",
...
/* ── 10-Year Horizon Milestones ──
   The phases carry the real expansion path: Gurugram today, the wider NCR
   next, pan-India after. */
function horizonMilestones(
  c: VisionCoverage,
): { era: string; focus: string; outcome: string }[] {
  return [
    {
      era: "Phase I · The Intelligence Layer",
      focus: "Establishing Ground Truth in Gurugram",
      outcome: `Building a verified record of project timelines, legal title audits, developer balance sheets, and live construction monitoring. Today that means ${coverageScope(
        c,
      )}.`,
    },
    {
      era: "Phase II · The Fiduciary Office",
      focus: "Full-Spectrum Buyer Protection Across NCR",
      outcome:
        "Extending that record across the wider National Capital Region, and giving domestic buyers, HNI families, and global NRIs a dedicated buyer's office—from unbiased shortlisting through contract negotiation and legal conveyance.",
    },
    {
      era: "Phase III · The Institutional Standard",
      focus: "A Pan-India Transparency Standard",
      outcome:
        "Taking the standard nationwide: an Indian real estate market that is transparent, liquid, and analytically rigorous, where capital flows to developers with proven execution integrity.",
    },
  ];
}

export default function VisionMission({ coverage = {} }: { coverage?: VisionCoverage }) {
  const { open } = useJourney();
  const { openConsult } = useConsultation();
  const MISSION_PILLARS = missionPillars(coverage);
  const HORIZON_MILESTONES = horizonMilestones(coverage);
        ...
        <div className="px-6 pb-12 md:px-12">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-[0.92rem] font-light leading-[1.9] text-white/35 md:text-[1rem]">
              In an industry built on sales brochures and asymmetric information,
              <br />
