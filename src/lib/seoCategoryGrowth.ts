...
export function clampTitle(t: string, maxLen = 60): string {
  const str = t.trim();
  if (str.length <= maxLen) return str;
  return str.slice(0, maxLen - 1).trim() + "…";
}

export function clampDescription(d: string, maxLen = 155): string {
  const str = d.trim();
  if (str.length <= maxLen) return str;
  return str.slice(0, maxLen - 1).trim() + "…";
}
...
export function getProjectSeoMeta(input: ProjectSeoInput): ProjectSeoOutput {
  ...
  if (overrideMap.has(key)) {
    ...
    return {
      title: clampTitle(matched.title),
      description: clampDescription(matched.desc),
