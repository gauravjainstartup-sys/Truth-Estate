/* ════════════════════════════════════════════════════════════════
   LIVE → REPORT ADAPTER (pure, browser-safe) — maps a scored pipeline
   row onto the existing ProjectIntel shape so live projects render
   through the ORIGINAL Project Report UI, untouched. Fields the pipeline
   hasn't extracted yet become "NA" (or stay absent so the report's own
   hide-when-missing behaviour applies). Pure data — no UI here.

   WHY ITS OWN MODULE (split out of liveReport.ts): this file imports NO
   Node-only code — only TYPES from supabase.ts (erased at compile) and
   browser-safe libs — so ONE source runs in BOTH places:
     · the build (liveReport.ts's buildLiveCatalog, the static pages), and
     · the browser (the live-data overlay — LiveProjectProfile).
   The fetch-bound builders (buildLiveCatalog / buildLiveMarket) stay in
   liveReport.ts, which re-exports liveProjectIntel / toMatchInput from
   here so no existing import path changes.
...
import { bhkBucket, type MatchInput, type UnitConfig } from "./matchEngine";
...
function mediaSrc(u: string | null | undefined, remote = false): string | null {
  if (!u) return null;
  ...
  // viaUrls already remapped anything pulled same-origin.
  //
  // AT BUILD (remote=false): never bake a raw Storage <img> — that is cached
  // egress on every page view — so return null and the component renders its
  // stand-in (founder rule: missing data hides, it never leaks).
  //
  // AT RUNTIME (remote=true, the live overlay): a raw Storage/R2 URL is exactly
  // a freshly-uploaded asset not yet materialized — the whole point of instant
  // updates — so hand it to the <img>. viaUrls above already remapped any
  // UNCHANGED asset to its cached same-origin file, so only genuinely-new assets
  // load from Storage. PDFs are unaffected either way (pdfSrc links them).
  if (isHttpUrl(s)) return remote && imageish(s) ? s : null;
...
function mediaPages(u: string | null | undefined, remote = false): string[] | null {
  if (!u) return null;
  const s = u.trim();
  let parts: string[] = [];
  if (s.startsWith("[")) {
  ...
  const pages = parts.map((p) => mediaSrc(viaUrls(p.trim()), remote)).filter((p): p is string => !!p);
...
export function matchKey<T>(id: string, name: string, table: Record<string, T> | null, nameIds: Record<string, string> | null, altIds: string[] = []): string | null {
...
/* buildLiveCatalog / buildLiveMarket live in liveReport.ts — they need the
...
export function liveProjectIntel(
  ...
  /* remoteMedia: allow raw Storage/R2 image URLs through to the <img> — the
     ...
     want to show. Default OFF, so build-time renders are byte-for-byte unchanged. */
  const rm = opts?.remoteMedia ?? false;
  /* prefer materialized files over inline blobs — but ONLY at build. At runtime
     the by-id manifest points at build-time assets, which would SHADOW a
     since-changed upload; skip it and let viaUrls remap unchanged urls per-URL. */
  const mm = !rm && extRaw ? MANIFEST[extRaw.backlogId] : undefined;
  ...
  const rawHomes = (cfgs ?? [])
    ...
    .map((c) => ({
      ...
      plans: mediaPages(c.floorPlanImageUrl, rm) ?? (mediaSrc(c.floorPlanImageUrl, rm) ? [mediaSrc(c.floorPlanImageUrl, rm)!] : []),
    }));
  ...
  const heroSrc = mediaSrc(ext?.heroImageUrl, rm);
  ...
  const renderSrc = mediaSrc(ext?.renderElevationUrl, rm);
  const siteMapSrc = mediaSrc(ext?.siteMapImageUrl, rm);
  const brochurePages = mediaPages(ext?.brochureUrl, rm) ?? (mediaSrc(ext?.brochureUrl, rm) ? [mediaSrc(ext?.brochureUrl, rm)!] : null);
  const brochurePdf = brochurePages ? null : pdfSrc(ext?.brochureUrl);
  const paymentSrc = mediaSrc(ext?.paymentPlanUrl, rm);
  ...
  if (ext) {
    ...
    // remoteMedia (the live overlay) loads Storage urls ON PURPOSE, so it is
    // never a "leak" there — only flag at build (rm=false).
    if ((inlineLeak && !rm) || DBG)
