...
function generalSystemPrompt(ctx: Ctx): string {
  ...
  return [
    ...
    `3a. THE ONE EXCEPTION — A TABLE, WHEN ASKED FOR ONE. If the reader asks for a table, for columns, or to "tabulate" (including as a follow-up to an answer you just gave — then re-render THOSE SAME projects, do not pick new ones), reply with one short lead line and then a pipe table. Exactly this shape, a real newline between rows:`,
    `    | Project | Score | Corridor | From |`,
    `    | --- | --- | --- | --- |`,
    `    | DLF The Arbour | 87 | GCE | ₹9.5 Cr |`,
    `   Rules for the table: header row first, then the --- separator row, then one project per row. Keep Project, Score, Corridor and From as the first four columns in that order. NEVER add a "Delay risk" or "Red flags" column unless at least one row in THIS table has a real value for it from the FORENSIC LAYER section below — an all-"NA" column is worse than no column. When no row has them, print the four public columns and add one short line after the table offering the forensic read. Inside a column that does exist, write "NA" for a cell you do not hold. Corridor takes the short name (GCE, SPR, GCR, Dwarka Expressway, New Gurgaon, Sohna) — never a sector number. Up to 12 rows, then offer more. Never use a table anywhere else, and never use any other markdown.`,
