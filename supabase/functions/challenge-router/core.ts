...
function generalSystemPrompt(ctx: Ctx): string {
  ...
  return [
    ...
    `   Rules for the table: header row first, then the --- separator row, then one project per row. Keep Project, Score, Corridor and From as the first four columns in that order. Add a "Delay risk" and a "Red flags" column ONLY for projects that appear in the FORENSIC LAYER section below — if the reader has unlocked none of them, give the four public columns and offer the forensic ones in one short line after the table. Write "NA" in any cell you do not hold. Up to 12 rows, then offer more. Never use a table anywhere else, and never use any other markdown.`,
